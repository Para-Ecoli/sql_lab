...
CREATE VIEW view1("s#",sname,bdate,height) AS
    SELECT s444."s#", s444.sname, s444.bdate, s444.height FROM s444 WHERE (((s444.sex)::text = '男'::text) AND ((s444.dorm)::text ~~ '东 18 舍%'::text));


ALTER VIEW public.view1 OWNER TO admin;

--
-- Name: view2; Type: TABLE; Schema: public; Owner: admin; Tablespace: 
--
...
ALTER TABLE ONLY (view2) REPLICA IDENTITY NOTHING;


ALTER TABLE public.view2 OWNER TO admin;

--
-- Name: view3; Type: VIEW; Schema: public; Owner: admin
--

CREATE VIEW view3("s#",sname,grade) AS
    SELECT s444."s#", s444.sname, sc444.grade FROM s444, c444, sc444 WHERE ((((s444."s#")::text = (sc444."s#")::text) AND ((c444."c#")::text = (sc444."c#")::text)) AND ((c444.cname)::text = '人工智能'::text));


ALTER VIEW public.view3 OWNER TO admin;
...
--
-- Name: _RETURN; Type: RULE; Schema: public; Owner: admin
--

SET enable_cluster_resize = on;
CREATE RULE "_RETURN" AS ON SELECT TO view2 DO INSTEAD SELECT c444."c#", c444.cname, avg(sc444.grade) AS avg_grade FROM c444, sc444 WHERE (((c444."c#")::text = (sc444."c#")::text) AND ((c444.teacher)::text = '张明'::text)) GROUP BY c444."c#";
